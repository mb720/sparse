...
object Parser {
  ...
  /**
   * When a line within the parsed text matches the `predicate`, we set a [[BlockMarker]] marking the beginning of a [[TextBlock]] at that line.
   * Example usage:
   *
  ...
  object from extends MarkerFactory {
    override def apply(predicate: ((String, Int) => Boolean)) = {
      BlockMarker(predicate)
    }
  }
  /**
   * When a line within the parsed text matches the `predicate`, we set a [[BlockMarker]] at that line. This marker indicates the end of a [[TextBlock]].
   * Example usage:
   *
   *  `parse(source, from("start line"), to("end line"))
   *  `parse(source, from(_.endsWith("start line suffix")), to(_.startsWith("end line prefix")))
  ...
  /**
   * When a line within the parsed text matches the `predicate`, we set a [[BlockMarker]] after that line. This marker may either indicate the start or end of a [[TextBlock]].
   * Example usage:
   *
  ...
  object after extends MarkerFactory {
    override def apply(predicate: ((String, Int) => Boolean)) = {
      BlockMarker(predicate, offset = +1)
    }
  }
  /**
   * When a line within the parsed text matches the `predicate`, we set a [[BlockMarker]] before that line. This marker indicates the start of a [[TextBlock]].
   * Example usage:
   *
  ...
  /**
   * When a line within the parsed text matches the `predicate`, we set a [[BlockMarker]] before that line. This marker indicates the end of a [[TextBlock]].
   * Example usage:
   *
  ...
  /**
   * Parses [[eu.matthiasbraun.sparse.TextBlock]]s from a `source`, such as a file. The start and end of a text block are defined by the `start` and `end` [[eu.matthiasbraun.sparse.BlockMarker]]s.
   * These markers match lines within the text. The caller can create those markers using [[eu.matthiasbraun.sparse.from]] or [[eu.matthiasbraun.sparse.until]], for example.
   * Example usage:
   * {{{
   * val file = new File("parse/this/file")
   * val blocksMaybe = parse(fromFile(file), from("startline"), to("endline"))
   *   blocksMaybe match {
   *     case Success(blocks)    => blocks.foreach { println }
   *     case Failure(exception) => println(exception)
   * }
   * }}}
   * @param source the [[Source]] that contains the text that we want to parse
   * @param start the text blocks we want to parse begin at this [[BlockMarker]]
   * @param end the text blocks we want to parse stop at this [[BlockMarker]]
   * @return either a list of [[TextBlock]]s wrapped in a [[Success]] or a [[Failure]] with the first exception that occurred during parsing.
   */
  def parse(source: Source, start: BlockMarker, end: BlockMarker): Try[List[TextBlock]] = {
    ...
    blocksMaybe.either match {
      case Left(exceptions) => Failure(exceptions(0))
